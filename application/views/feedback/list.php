...
<div class="right_countainer">
    ...
    <div class="impbutton">
        <!--<a href="<?php echo base_url('feedback/add') ?>" title="Add New" class="btn btn-primary btn-sm"><i class="icon-search icon-white"></i>Add New</a>-->
                ...
                <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered datatable" id="area">
                    ...
                    <tbody>
                        <?php foreach ($data_all->data as $key => $dataVal) { ?>
                            <tr class="odd gradeX">
                                <td><?php echo $dataVal->user_name; ?></td>
                                <td><?php echo $dataVal->user_contact_number; ?></td>
                                <td><?php echo $dataVal->distributor_name; ?></td>
                                <td><?php echo $dataVal->distributor_contact_number; ?></td>
                                <td><?php echo $dataVal->order_id; ?></td>
                                <td><?php echo $dataVal->feedback_msg; ?></td>
                                <td><?php echo $dataVal->rating; ?></td>
...
<?php $posturl = base_url() . 'feedback/getDataList'; ?>
<!--modal view-->
<script>
...
<script>
    $(document).ready(function () {
        ...
        $('.datatable ').on("click", '.view', function () { //any Click on Delete icon
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url('feedback/view'); ?>",
                data: {postID: postVal},
        ...
        $('.datatable').dataTable()
                .columnFilter({
                    aoColumns: [
                        ...
                        null,
                        null,
                        {type: "select", values: [<?php foreach($allorders as $orderVal){ echo "'".$orderVal->id."',";}?>]},
                        null,
                        null,
                        null
                    ]
